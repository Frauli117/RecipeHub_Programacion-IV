...
import { crearReceta, actualizarReceta, obtenerReceta } from "../api/recetaApi";
...
export default function FormReceta() {
    const { abierto, modo, recetaId, onGuardado, cerrar } = useModalReceta();
    ...
    useEffect(() => {
        ...
        const cargarDatos = async () => {
            if (modo === "editar" && recetaId) {
                setCargando(true);
                try {
                    const receta = await obtenerReceta(recetaId);

                    // Si el componente cambió de estado o se cerró mientras esperábamos, no hacemos nada
                    if (!activo) return;
                    ...
                    // 🌟 SI LA PETICIÓN FUE ABORTADA POR EL CIERRE, IGNORAMOS EL ERROR
                    if (!activo) return;
    ...
    const handleGuardar = async () => {
        ...
        try {
            let resultado;
            if (modo === "editar") {
                resultado = await actualizarReceta(recetaId, payload);
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={modo === "editar" ? "Editar receta" : "Nueva receta"}
        >
